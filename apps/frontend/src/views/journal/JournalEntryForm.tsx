 ...
 * フォーム状態管理フック
 */
const useJournalEntryFormState = () => {
  const lineIdRef = useRef(0);
  const [journalDate, setJournalDate] = useState('');
  const [description, setDescription] = useState('');
  const [lines, setLines] = useState<JournalEntryLineState[]>([createEmptyLine(0)]);
  ...
  const buildRequestPayload = (): CreateJournalEntryRequest => ({
    journalDate,
...
export const JournalEntryForm: React.FC<JournalEntryFormProps> = ({
  ...
  const form = useJournalEntryFormState();
