...
- [ ] US-FS-002 の全受入条件を満たしている
- [ ] テストカバレッジ 80% 以上を維持
- [ ] SonarQube Quality Gate パス
- [ ] リリース 2.0 全機能の統合テスト完了
...
|----------|---------|---------|------|------|
| FS-002-01 | 損益計算書ユースケース設計 | 2h | Claude | [ ] |
| FS-002-02 | GetProfitAndLossQuery / Result 実装 | 2h | Codex | [ ] |
| FS-002-03 | GetProfitAndLossService 実装（利益計算ロジック） | 5h | Codex | [ ] |
| FS-002-04 | ProfitAndLossRepository（Output Port）+ MyBatis mapper | 3h | Codex | [ ] |
| FS-002-05 | 前期比較ロジック実装 | 3h | Codex | [ ] |
| FS-002-06 | 損益計算書 API 実装（GET /api/profit-and-loss） | 2h | Codex | [ ] |
| FS-002-07 | 単体テスト作成 | 4h | Codex | [ ] |
| FS-002-08 | 損益計算書画面 UI 実装（報告式レイアウト） | 6h | Codex | [ ] |
| FS-002-09 | 前期比較 UI 実装 | 3h | Codex | [ ] |
| FS-002-10 | PDF/Excel ダウンロード機能実装 | 4h | Codex | [ ] |
| FS-002-11 | フロントエンド API 連携 | 2h | Codex | [ ] |
| FS-002-12 | 統合テスト（E2E テスト） | 3h | Claude | [ ] |
| | **小計** | **39h** | | |
...
- [ ] US-FS-002 の受入条件を理解した
- [ ] タスク分解と見積もりに合意した
- [ ] 8SP を2週間で完了することにコミットする
- [ ] リリース 2.0 の統合テストを実施する
- [ ] デイリースタンドアップを毎日実施する
- [ ] 問題発生時は即座に共有する
