...
- [x] 勘定科目を選択して月次残高を表示できる
- [x] 月、借方合計、貸方合計、残高が表示される
- [x] 年度を指定して絞り込みできる
- [x] 月次推移をグラフで表示できる

| タスクID | タスク名 | 理想時間 | 担当 | 状態 |
|----------|---------|---------|------|------|
| LDG-004-01 | 月次残高照会ユースケース設計 | 2h | Claude | 完了 |
| LDG-004-02 | GetMonthlyBalanceQuery 実装 | 2h | Codex | 完了 |
| LDG-004-03 | GetMonthlyBalanceService 実装 | 4h | Codex | 完了 |
| LDG-004-04 | 月次残高 API 実装（GET /api/monthly-balance） | 3h | Codex | 完了 |
| LDG-004-05 | 単体テスト作成 | 3h | Codex | 完了 |
| LDG-004-06 | 月次残高照会画面 UI 実装 | 6h | Codex | 完了 |
| LDG-004-07 | 月次推移グラフ実装 | 4h | Codex | 完了 |
| LDG-004-08 | フロントエンド API 連携 | 2h | Codex | 完了 |
| LDG-004-09 | 統合テスト（E2E テスト） | 2h | Claude | 完了 |
| | **小計** | **28h** | | |
...
- [x] 全勘定科目の借方残高、貸方残高が一覧表示される
- [x] 借方合計と貸方合計が一致していることを確認できる
- [x] 基準日を指定して残高を計算できる
- [x] 勘定科目種別ごとの小計が表示される

| タスクID | タスク名 | 理想時間 | 担当 | 状態 |
|----------|---------|---------|------|------|
| LDG-005-01 | 残高試算表ユースケース設計 | 2h | Claude | 完了 |
| LDG-005-02 | GetTrialBalanceQuery 実装 | 2h | Codex | 完了 |
| LDG-005-03 | GetTrialBalanceService 実装 | 5h | Codex | 完了 |
| LDG-005-04 | 貸借一致検証ロジック実装 | 3h | Codex | 完了 |
| LDG-005-05 | 残高試算表 API 実装（GET /api/trial-balance） | 3h | Codex | 完了 |
| LDG-005-06 | 単体テスト作成 | 3h | Codex | 完了 |
| LDG-005-07 | 残高試算表画面 UI 実装 | 8h | Codex | 完了 |
| LDG-005-08 | 勘定科目種別ごとの小計表示 | 3h | Codex | 完了 |
| LDG-005-09 | フロントエンド API 連携 | 2h | Codex | 完了 |
| LDG-005-10 | 統合テスト（E2E テスト） | 2h | Claude | 完了 |
| | **小計** | **33h** | | |
...
| 日 | 主要タスク | 状態 |
|----|-----------|------|
| 火(2/10) | 仕訳差し戻し全タスク完了（3SP）、仕訳確定全タスク完了（5SP）、月次残高照会全タスク完了（5SP） | ✅ 完了 |
| 水(2/11) | （前倒し完了により空き） | - |
| 木(2/12) | （前倒し完了により空き） | - |
| 金(2/13) | 残高試算表表示全タスク完了（6SP） | ✅ 完了 |
| 土(2/14) | （前倒し完了により空き） | - |
...
| 日 | 主要タスク | 状態 |
|----|-----------|------|
| 月(2/16) | 共通タスク消化（COMMON-18, 19） | 予定 |
| 火(2/17) | 共通タスク消化（COMMON-23） | 予定 |
| 水(2/18) | 共通タスク消化（COMMON-24） | 予定 |
| 木(2/19) | 共通タスク消化（COMMON-25） | 予定 |
| 金(2/20) | バッファ | 予定 |
| 土(2/21) | - | - |
| 日(2/22) | - | - |
| 月(2/23) | バッファ | 予定 |
| 火(2/24) | 全体統合テスト、イテレーション完了 | 予定 |
...
| 日 | 計画残時間 | 実績残時間 | 差異 |
|----|-----------|-----------|------|
| Day 1 (2/10) | 130h | 51h | +79h（JNL-009 23h + JNL-010 28h + LDG-004 28h 完了） |
| Day 2 (2/11) | 117h | - | - |
| Day 3 (2/12) | 104h | - | - |
| Day 4 (2/13) | 91h | 18h | +73h（LDG-005 33h 完了、残り共通タスク 18h のみ） |
| Day 5 (2/14) | 78h | - | - |
| Day 6 (2/16) | 65h | - | - |
| Day 7 (2/17) | 52h | - | - |
| Day 8 (2/18) | 39h | - | - |
...
- [x] 月次残高照会 ← **イテレーション 6 で完了（2026-02-10）**
- [x] 残高試算表表示 ← **イテレーション 6 で完了（2026-02-13）**
...
| 2026-02-10 | US-LDG-004 月次残高照会完了（5SP）、Day 1 で 13SP 達成 | Claude Opus 4.6 |
| 2026-02-13 | US-LDG-005 残高試算表表示完了（6SP）、全 4 ストーリー 19SP 達成。進捗状況を一括更新 | Claude Opus 4.6 |
| 2026-02-13 | 全 4 ストーリー 19SP 完了確認。US-LDG-002 は IT-7 に据え置き | Claude Opus 4.6 |
