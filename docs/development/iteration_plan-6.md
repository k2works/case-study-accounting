...
| タスクID | タスク名 | 理想時間 | 担当 | 状態 |
|----------|---------|---------|------|------|
| JNL-010-01 | 仕訳確定ユースケース設計 | 2h | Claude | 未着手 |
| JNL-010-02 | ConfirmJournalEntryCommand 実装 | 2h | Codex | 未着手 |
| JNL-010-03 | ConfirmJournalEntryService 実装 | 4h | Codex | 未着手 |
| JNL-010-04 | 元帳転記ロジック実装（PostToLedgerService） | 6h | Codex | 未着手 |
| JNL-010-05 | 確定済み仕訳の編集・削除防止バリデーション | 2h | Codex | 未着手 |
| JNL-010-06 | 確定 API 実装（POST /api/journal-entries/{id}/confirm） | 2h | Codex | 未着手 |
| JNL-010-07 | 単体テスト作成 | 3h | Codex | 未着手 |
| JNL-010-08 | 確定ボタン UI 実装 | 3h | Codex | 未着手 |
| JNL-010-09 | フロントエンド API 連携 | 2h | Codex | 未着手 |
| JNL-010-10 | 統合テスト（E2E テスト） | 2h | Claude | 未着手 |
| | **小計** | **28h** | | |
...
| 日 | 主要タスク | 状態 |
|----|-----------|------|
| 火(2/10) | 仕訳差し戻し設計、共通タスク（リリースノート・API ドキュメント） | 予定 |
| 水(2/11) | 仕訳差し戻し Command/Service 実装、仕訳確定設計 | 予定 |
| 木(2/12) | 仕訳差し戻しドメインモデル拡張・DB マイグレーション | 予定 |
| 金(2/13) | 仕訳差し戻し API 実装・単体テスト | 予定 |
| 土(2/14) | 仕訳差し戻し UI・E2E、仕訳確定 Command/Service、ステータス遷移図 | 予定 |
