...
- [x] ユーザーID、氏名、ロール、最終ログイン日時が一覧表示される
- [x] ロールでフィルタリングできる
- [x] ユーザーIDまたは氏名で検索できる
- [x] 一覧から詳細画面に遷移できる
...
| タスクID | タスク名 | 理想時間 | 担当 | 状態 |
|----------|---------|---------|------|------|
| AUTH-006-01 | ユーザー一覧ユースケース設計 | 2h | Claude | 完了 |
| AUTH-006-02 | GetUsersQuery 実装 | 2h | Codex | 完了 |
| AUTH-006-03 | GetUsersService 実装 | 4h | Codex | 完了 |
| AUTH-006-04 | ユーザー一覧 API 実装（GET /api/users） | 3h | Codex | 完了 |
| AUTH-006-05 | フィルタリング・検索ロジック実装 | 3h | Codex | 完了 |
| AUTH-006-06 | 単体テスト作成 | 3h | Codex | 完了 |
| AUTH-006-07 | ユーザー一覧画面 UI 実装 | 6h | Codex | 完了 |
| AUTH-006-08 | フィルタ・検索 UI 実装 | 3h | Claude | 完了 |
| AUTH-006-09 | フロントエンド API 連携 | 2h | Codex | 完了 |
| AUTH-006-10 | 統合テスト（E2E テスト） | 2h | Claude | 完了 |
| | **小計** | **30h** | | |
...
- [x] 「下書き」ステータスの仕訳のみ承認申請できる
- [x] 承認申請後、ステータスが「承認待ち」に変わる
- [x] 承認申請成功時、確認メッセージが表示される

| タスクID | タスク名 | 理想時間 | 担当 | 状態 |
|----------|---------|---------|------|------|
| JNL-007-01 | 仕訳承認申請ユースケース設計 | 2h | Claude | 完了 |
| JNL-007-02 | SubmitForApprovalCommand 実装 | 2h | Codex | 完了 |
| JNL-007-03 | SubmitForApprovalService 実装 | 3h | Codex | 完了 |
| JNL-007-04 | 仕訳ステータス遷移ロジック実装 | 3h | Codex | 完了 |
| JNL-007-05 | 承認申請 API 実装（POST /api/journal-entries/{id}/submit） | 2h | Codex | 完了 |
| JNL-007-06 | 単体テスト作成 | 2h | Codex | 完了 |
| JNL-007-07 | 承認申請ボタン UI 実装 | 2h | Codex | 完了 |
| JNL-007-08 | フロントエンド API 連携 | 2h | Codex | 完了 |
| JNL-007-09 | 統合テスト（E2E テスト） | 2h | Claude | 完了 |
| | **小計** | **20h** | | |
...
- [x] 「承認待ち」ステータスの仕訳のみ承認できる
- [x] 承認後、ステータスが「承認済み」に変わる
- [x] 承認者と承認日時が記録される
- [x] 承認成功時、確認メッセージが表示される

| タスクID | タスク名 | 理想時間 | 担当 | 状態 |
|----------|---------|---------|------|------|
| JNL-008-01 | 仕訳承認ユースケース設計 | 2h | Claude | 完了 |
| JNL-008-02 | ApproveJournalEntryCommand 実装 | 2h | Codex | 完了 |
| JNL-008-03 | ApproveJournalEntryService 実装 | 3h | Codex | 完了 |
| JNL-008-04 | 承認者・承認日時記録ロジック実装 | 2h | Codex | 完了 |
| JNL-008-05 | 承認 API 実装（POST /api/journal-entries/{id}/approve） | 2h | Codex | 完了 |
| JNL-008-06 | 単体テスト作成 | 2h | Codex | 完了 |
| JNL-008-07 | 承認画面 UI 実装 | 3h | Codex | 完了 |
| JNL-008-08 | フロントエンド API 連携 | 2h | Codex | 完了 |
| JNL-008-09 | 統合テスト（E2E テスト） | 2h | Codex | 完了 |
| | **小計** | **20h** | | |
...
- [x] ユーザー管理（編集・削除・一覧）
- [x] 仕訳承認ワークフロー（申請・承認）※差し戻し・確定はイテレーション 6 で実装予定
- [ ] 月次残高照会
- [ ] 残高試算表表示
- [ ] 貸借対照表表示
- [ ] 損益計算書表示
...
| 2026-02-07 | US-AUTH-006 完了（ユーザー一覧表示、フィルタ・検索機能） | Claude Opus 4.5 |
| 2026-02-09 | US-JNL-007 完了（仕訳承認申請機能、E2E テスト含む） | Claude Opus 4.5 |
| 2026-02-09 | US-JNL-008 完了（仕訳承認機能、DB マイグレーション、E2E テスト含む） | Claude Opus 4.5 |
