...
```mermaid
gantt
    title 財務会計システム 実績スケジュール
    dateFormat  YYYY-MM-DD

    section リリース1.0 MVP
    IT1 認証・マスタ (14d)     :done, a1, 2026-01-05, 14d
    IT2 マスタ・仕訳入力 (6d)  :done, a2, 2026-01-21, 6d
    IT3 仕訳管理 (7d)          :done, a3, 2026-01-27, 7d
    IT4 元帳照会 (2d)          :done, a4, 2026-02-05, 2d
    リリース1.0 MVP            :milestone, ma1, 2026-02-06, 0d

    section リリース2.0 機能拡張版
    IT5 ユーザー管理・承認 (3d) :done, a5, 2026-02-07, 3d
    IT6 差戻・確定・残高 (4d)   :done, a6, 2026-02-10, 4d
    IT7 貸借対照表・補助元帳 (3d) :done, a7, 2026-02-14, 3d
    IT8 損益計算書 (1d)          :done, a8, 2026-02-17, 1d
    リリース2.0 機能拡張版      :milestone, ma2, 2026-02-17, 0d

    section リリース3.0 完成版
    IT9 マスタ拡張 (2d)          :done, a9, 2026-02-18, 2d
    IT10 自動仕訳・財務分析 (2d)   :done, a10, 2026-02-20, 2d
    IT11 監査ログ・データDL (2d)   :done, a11, 2026-02-22, 2d
    リリース3.0 全機能実装完了      :milestone, ma3f, 2026-02-23, 0d
    IT12 品質保証・リリース準備 (1d) :done, a12, 2026-02-24, 1d
    リリース3.0 完成版              :milestone, ma3, 2026-02-24, 0d
```
...
| 7 | 13 | 13 | 111 | 44 | ✅ 完了（貸借対照表・補助元帳・共通タスク COMMON-18〜25 完了） |
| 8 | 8 | 8 | 119 | 36 | ✅ 完了（損益計算書表示完了、リリース 2.0 機能拡張版達成） |
| 9 | 13 | 13 | 132 | 23 | ✅ 完了（勘定科目構成登録・編集、自動仕訳パターン CRUD） |
| 10 | 10 | 10 | 142 | 13 | ✅ 完了（自動仕訳生成・財務分析表示完了） |
| 11 | 13 | 13 | 155 | 0 | ✅ 完了（監査ログ照会・データダウンロード完了） |
| 12 | 10 | 10 | 165 | 0 | ✅ 完了（統合テスト・リリース準備・品質保証完了） |
...
| ストーリーID | ストーリー名 | SP | バッファ対象 | 状態 |
|-------------|-------------|-----|------------|------|
| US-MST-005 | 勘定科目構成登録 | 5 | ○ | ✅ IT-9 消化 |
| US-MST-006 | 勘定科目構成編集 | 3 | ○ | ✅ IT-9 消化 |
| US-MST-007 | 自動仕訳設定登録 | 3 | ○ | ✅ IT-9 消化 |
| US-MST-008 | 自動仕訳設定編集 | 2 | ○ | ✅ IT-9 消化 |
| US-JNL-006 | 自動仕訳生成 | 5 | ○ | ✅ IT-10 消化 |
| US-FS-003 | 財務分析表示 | 5 | ○ | ✅ IT-10 消化 |
| US-SYS-002 | データダウンロード | 5 | ○ | ✅ IT-11 消化 |
| **合計** | | **28** | | **28SP 全消化済み** |

**フィーチャバッファ比率**: 28 / 155 = 18%（全フィーチャバッファ消化済み。残バッファ 0SP）
...
| リスク | 影響 | 対策 | 状態 |
|-------|------|------|------|
| JWT認証の実装 | 中 | イテレーション1でスパイク実施 | ✅ 解決済み |
| 貸借一致検証ロジック | 高 | 早期にドメインモデル検証 | ✅ 解決済み（残高試算表で検証実装） |
| 財務諸表計算の複雑性 | 高 | 段階的な実装とレビュー | ✅ 解決済み（貸借対照表 IT-7、損益計算書 IT-8 完了） |
| パフォーマンス | 中 | 各イテレーションで計測 | ✅ 問題なし |
...
- [x] 貸借対照表を出力できる（イテレーション 7 完了 2026-02-14）
- [x] 損益計算書を出力できる（イテレーション 8 完了 2026-02-17）
- [x] 残高試算表で貸借一致を確認できる（残高試算表表示完了 2026-02-13）
- [x] 補助元帳を照会できる（補助元帳照会完了 2026-02-14）
- [x] 月次残高を照会できる（月次残高照会完了 2026-02-10）
- [x] ユーザー管理ができる（ユーザー編集・削除・一覧完了 2026-02-07）

### リリース3.0

- [x] 勘定科目構成を管理できる（勘定科目構成登録・編集完了 2026-02-19）
- [x] 自動仕訳パターンを管理できる（自動仕訳パターン CRUD 完了 2026-02-19）
- [x] 自動仕訳が機能する（自動仕訳生成完了 2026-02-21）
- [x] 財務分析指標を確認できる（財務分析表示完了 2026-02-21）
- [x] 監査ログを照会できる（監査ログ照会完了 2026-02-23）
- [x] 各種データをダウンロードできる（データダウンロード完了 2026-02-23）
...
| 2026-02-13 | 2.5 | イテレーション6全完了確認（19SP/19SP）。US-LDG-002 補助元帳照会は IT-7 に据え置き | Claude Opus 4.6 |
| 2026-02-14 | 2.6 | イテレーション7計画作成（13SP + 共通タスク 18h、US-FS-001 貸借対照表・US-LDG-002 補助元帳・COMMON-18〜25 消化） | Claude Opus 4.6 |
| 2026-02-14 | 2.7 | イテレーション7進捗更新（US-FS-001 貸借対照表表示 8SP 完了。バックエンド API + フロントエンド UI + PDF/Excel エクスポート実装） | Claude Opus 4.6 |
| 2026-02-14 | 2.8 | イテレーション7進捗更新（US-LDG-002 補助元帳照会 5SP 完了。バックエンド API + フロントエンド UI + E2E テスト実装） | Claude Opus 4.6 |
| 2026-02-16 | 2.9 | **イテレーション7全完了**（13SP/13SP 達成率100%）。COMMON-18〜25 共通タスク全完了（リリースノート作成、ステータス遷移図、フロントエンド権限チェック。API ドキュメント・E2E CI は既に実装済み確認） | Claude Opus 4.6 |
| 2026-02-17 | 3.0 | イテレーション7ふりかえり・イテレーション8計画作成（8SP、US-FS-002 損益計算書表示）。GitHub Issue #31, #27 クローズ、Project 同期。リリース 2.0 最終イテレーション開始 | Claude Opus 4.6 |
| 2026-02-17 | 3.1 | **イテレーション8完了・リリース 2.0 達成**（8SP/8SP 達成率100%）。US-FS-002 損益計算書表示完了。GitHub Issue #32 クローズ、Project Board 同期（#32-35 追加）、Milestone「リリース 1.0 MVP」クローズ。バーンダウン更新（残 36SP） | Claude Opus 4.6 |
| 2026-02-19 | 3.2 | イテレーション9進捗（US-MST-006 勘定科目構成編集完了、8SP/13SP）。GitHub Issue #13 クローズ、Project Board 同期 | Claude Opus 4.6 |
| 2026-02-19 | 3.3 | **イテレーション9全完了**（13SP/13SP 達成率100%）。US-MST-007/008 自動仕訳パターン CRUD 完了。GitHub Issue #14, #15 クローズ、Project Board 同期。バーンダウン更新（残 23SP） | Claude Opus 4.6 |
| 2026-02-21 | 3.4 | **イテレーション10全完了**（10SP/10SP 達成率100%）。US-JNL-006 自動仕訳生成・US-FS-003 財務分析表示完了。GitHub Issue #21, #33 クローズ、Project Board 同期。バーンダウン更新（残 13SP） | Claude Opus 4.6 |
| 2026-02-23 | 3.5 | **イテレーション11全完了**（13SP/13SP 達成率100%）。US-SYS-001 監査ログ照会・US-SYS-002 データダウンロード完了。全機能ストーリー（155SP）実装完了。バーンダウン更新（残 0SP）。IT-12 は統合テスト・リリース準備に充当 | Claude Opus 4.6 |
| 2026-02-24 | 3.6 | **イテレーション12全完了・リリース 3.0 完成版達成**。統合テスト全パス（2,052テスト）、SonarQube BLOCKER修正、E2E安定性ガイドライン作成、リリースノートv3.0作成、GitHub全Milestone クローズ、Project Board全Done。プロジェクト完了 | Claude Opus 4.6 |
