...
| US-MST-006 | 勘定科目構成編集 | 3 | 重要 | ✅ 完了 |
| US-MST-007 | 自動仕訳設定登録 | 3 | 重要 | ✅ 完了 |
| US-MST-008 | 自動仕訳設定編集 | 2 | 重要 | ✅ 完了 |
| **合計** | | **13** | | **13SP 完了** |
...
- [x] US-MST-005, 006, 007, 008 の全受入条件を満たしている
- [x] テストカバレッジ 80% 以上を維持
...
- [x] 設定名、借方科目、貸方科目、デフォルト金額、摘要を入力して登録できる
- [x] 登録成功時、確認メッセージが表示される
...
- [x] 設定名、借方科目、貸方科目、デフォルト金額、摘要を編集できる
- [x] 編集成功時、確認メッセージが表示される
...
| MST-006-07 | 統合テスト（E2E テスト） | 2h | Claude | [x] |
| | **小計** | **16h** | | |

---

### US-MST-007: 自動仕訳設定登録（3SP）✅ 完了

| タスクID | タスク名 | 理想時間 | 担当 | 状態 |
|----------|---------|---------|------|------|
| MST-007-01 | 自動仕訳設定ユースケース設計 | 2h | Claude | [x] |
| MST-007-02 | AutoJournalPattern ドメインモデル実装 | 3h | Codex | [x] |
| MST-007-03 | CreateAutoJournalPatternCommand / Service 実装 | 3h | Codex | [x] |
| MST-007-04 | AutoJournalPatternRepository（Output Port）+ MyBatis mapper | 3h | Codex | [x] |
| MST-007-05 | 自動仕訳設定 API 実装（POST /api/auto-journal-patterns） | 2h | Codex | [x] |
| MST-007-06 | 自動仕訳設定一覧 API 実装（GET /api/auto-journal-patterns） | 2h | Codex | [x] |
| MST-007-07 | 単体テスト作成 | 3h | Codex | [x] |
| MST-007-08 | 自動仕訳設定登録画面 UI 実装 | 4h | Codex | [x] |
| MST-007-09 | フロントエンド API 連携 | 2h | Codex | [x] |
| MST-007-10 | 統合テスト（E2E テスト） | 2h | Claude | [x] |
| | **小計** | **26h** | | |
...
### US-MST-008: 自動仕訳設定編集（2SP）✅ 完了

| タスクID | タスク名 | 理想時間 | 担当 | 状態 |
|----------|---------|---------|------|------|
| MST-008-01 | UpdateAutoJournalPatternCommand / Service 実装 | 2h | Codex | [x] |
| MST-008-02 | 編集 API 実装（PUT /api/auto-journal-patterns/{id}） | 2h | Codex | [x] |
| MST-008-03 | 削除 API 実装（DELETE /api/auto-journal-patterns/{id}） | 2h | Codex | [x] |
| MST-008-04 | 単体テスト作成 | 2h | Codex | [x] |
| MST-008-05 | 自動仕訳設定編集画面 UI 実装 | 3h | Codex | [x] |
| MST-008-06 | フロントエンド API 連携 | 2h | Codex | [x] |
| MST-008-07 | 統合テスト（E2E テスト） | 2h | Claude | [x] |
| | **小計** | **15h** | | |
...
| タスクID | タスク名 | 理想時間 | 担当 | 状態 |
|----------|---------|---------|------|------|
| CARRY-01 | SonarQube Quality Gate 確認・記録 | 2h | Claude | [ ] (IT-10 に持ち越し) |
| | **小計** | **2h** | | |
...
| /master/auto-journal-patterns | 自動仕訳パターン一覧 | ✅ 実装済み |
| /master/auto-journal-patterns/new | 自動仕訳パターン登録 | ✅ 実装済み |
| /master/auto-journal-patterns/:id/edit | 自動仕訳パターン編集 | ✅ 実装済み |
...
- [x] 勘定科目構成編集 ← **イテレーション 9 で実装（2026-02-19 完了）**
- [x] 自動仕訳設定登録 ← **イテレーション 9 で実装（2026-02-19 完了）**
- [x] 自動仕訳設定編集 ← **イテレーション 9 で実装（2026-02-19 完了）**
...
- [ ] SonarQube Quality Gate パス（IT-10 に持ち越し）
- [x] ドキュメントが更新されている
...
| 2026-02-19 | US-MST-006 完了（E2E テスト 6 件追加、全 19 テストパス）、GitHub Project 同期 | Claude Opus 4.6 |
| 2026-02-19 | **イテレーション 9 全完了**（13SP/13SP 達成率 100%）。US-MST-007/008 自動仕訳パターン CRUD 完了。全タスク・受入条件チェック済み | Claude Opus 4.6 |
