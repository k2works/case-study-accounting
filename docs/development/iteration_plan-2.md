...
### US-MST-004: 勘定科目一覧表示（5SP） ✅ 完了（2026-01-26）

**受入条件**:

- [x] 科目コード、科目名、勘定科目種別が一覧表示される
- [x] 勘定科目種別でフィルタリングできる
- [x] 科目コードまたは科目名で検索できる
- [ ] 階層構造がある場合、ツリー形式で表示できる（将来拡張）

**注記**: 基本的な一覧表示はイテレーション1で実装済み。本イテレーションではフィルタリングと検索機能を追加。

| タスクID | タスク名 | 理想時間 | 担当 | 状態 |
|----------|---------|---------|------|------|
| MST-004-01 | 勘定科目検索クエリ実装（バックエンド） | 3h | Codex | 完了 |
| MST-004-02 | 勘定科目検索 API 実装 | 2h | Codex | 完了 |
| MST-004-03 | 単体テスト作成 | 2h | Codex | 完了 |
| MST-004-04 | 検索フォーム UI 実装 | 3h | Codex | 完了 |
| MST-004-05 | フィルタリング機能 UI 実装 | 3h | Codex | 完了 |
| MST-004-06 | フロントエンド API 連携 | 2h | Codex | 完了 |
| MST-004-07 | ソート機能実装 | 2h | - | スキップ（将来拡張） |
| MST-004-08 | ページネーション実装 | 3h | - | スキップ（将来拡張） |
| MST-004-09 | 統合テスト | 2h | Claude | 完了 |
| | **小計** | **22h** | | |
...
- `AccountMapper.search()` - 検索条件に基づく勘定科目検索クエリ
- `AccountRepository.search()` - Output Port に検索メソッド追加
- `AccountRepositoryImpl.search()` - 検索実装
- `AccountController.findAll()` - クエリパラメータ（type, keyword）対応

**フロントエンド実装**:

- `AccountFilter.tsx` - 検索・フィルタ UI コンポーネント
- `AccountFilter.css` - フィルタ用スタイル
- `getAccounts.ts` - API クライアントにクエリパラメータ対応追加
- `AccountList.tsx` - AccountFilter コンポーネント統合
- `AccountListPage.tsx` - フィルタ状態管理と検索ハンドラ

**テスト**:

- `AccountRepositorySearchTest` - リポジトリ検索テスト
- `AccountSearchApiIntegrationTest` - 検索 API 統合テスト
- `AccountList.test.tsx` - フロントエンドテスト更新

---

---

### US-JNL-001: 仕訳入力（8SP） ✅ 完了（2026-01-26）

**受入条件**:

- [x] 仕訳日付、摘要を入力できる
- [x] 借方科目、借方金額、貸方科目、貸方金額を入力できる
- [x] 複数の明細行を追加できる
- [x] 貸借の合計金額が一致しないと保存できない
- [x] 存在しない勘定科目は選択できない
- [x] 保存成功時、確認メッセージが表示される
- [x] 保存された仕訳のステータスは「下書き」になる

| タスクID | タスク名 | 理想時間 | 担当 | 状態 |
|----------|---------|---------|------|------|
| JNL-001-01 | 仕訳ドメインモデル設計 | 4h | Codex | 完了 |
| JNL-001-02 | JournalEntry エンティティ実装 | 4h | Codex | 完了 |
| JNL-001-03 | JournalEntryLine 値オブジェクト実装 | 3h | Codex | 完了 |
| JNL-001-04 | Money 値オブジェクト実装 | 2h | Codex | 完了 |
| JNL-001-05 | JournalEntryStatus 列挙型実装 | 1h | Codex | 完了 |
| JNL-001-06 | 貸借一致バリデーション実装 | 4h | Codex | 完了 |
| JNL-001-07 | JournalEntryRepository 実装 | 4h | Codex | 完了 |
| JNL-001-08 | 仕訳入力ユースケース実装 | 4h | Codex | 完了 |
| JNL-001-09 | 仕訳入力 API 実装 | 3h | Codex | 完了 |
| JNL-001-10 | DB マイグレーション作成 | 2h | Codex | 完了 |
| JNL-001-11 | 単体テスト作成 | 6h | Codex | 完了 |
| JNL-001-12 | 仕訳入力画面 UI 実装 | 8h | Codex | 完了 |
| JNL-001-13 | 明細行の動的追加/削除 UI 実装 | 4h | Codex | 完了 |
| JNL-001-14 | 科目選択コンポーネント実装 | 4h | Codex | 完了 |
| JNL-001-15 | バリデーション表示実装 | 3h | Codex | 完了 |
| JNL-001-16 | フロントエンド API 連携 | 3h | Codex | 完了 |
| JNL-001-17 | 統合テスト | 4h | Claude | 完了 |
| | **小計** | **63h** | | |
...
| US-MST-004 | 勘定科目一覧表示 | 5 | 完了 | 2026-01-26 |
| US-JNL-001 | 仕訳入力 | 8 | 完了 | 2026-01-26 |

### 日次残理想時間

| 日 | 計画残時間 | 実績残時間 | 差異 |
|----|-----------|-----------|------|
| Day 1 (01/21) | 119h | 105h | +14h |
| Day 2 (01/22) | 107h | 90h | +17h |
| Day 3 (01/23) | 95h | 70h | +25h |
| Day 4 (01/24) | 83h | 45h | +38h |
| Day 5 (01/25) | 71h | 20h | +51h |
| Day 6 (01/26) | 59h | 0h | +59h |
| Day 7 | - | - | - |
| Day 8 | - | - | - |
| Day 9 | - | - | - |
| Day 10 | - | - | - |

**注記**: イテレーション 2 は予定より 4 日早く完了（16SP 達成）
...
| 2026-01-26 | US-MST-004（勘定科目一覧表示）完了 | Claude Opus 4.5 + Codex |
| 2026-01-26 | US-JNL-001（仕訳入力）完了、イテレーション 2 完了 | Claude Opus 4.5 + Codex |
