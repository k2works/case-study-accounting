...
|----------|---------|---------|------|------|
| LDG-001-01 | 元帳照会ユースケース設計 | 2h | AI | 完了 |
| LDG-001-02 | GetGeneralLedgerQuery 実装 | 2h | AI | 完了 |
| LDG-001-03 | GetGeneralLedgerService 実装 | 4h | AI | 完了 |
| LDG-001-04 | 仕訳→元帳転記ロジック実装 | 4h | AI | 完了 |
| LDG-001-05 | 残高計算ロジック実装 | 3h | AI | 完了 |
| LDG-001-06 | 元帳照会 API 実装（GET /api/general-ledger） | 3h | AI | 完了 |
| LDG-001-07 | 単体テスト作成 | 4h | AI | 完了 |
| LDG-001-08 | 元帳照会画面 UI 実装 | 6h | AI | 完了 |
| LDG-001-09 | フロントエンド API 連携 | 2h | AI | 完了 |
| LDG-001-10 | 統合テスト（E2E テスト） | 2h | AI | 完了 |
| | **小計** | **32h** | | |
...
- [x] 勘定科目を選択して日次残高を表示できる
- [x] 日付、借方合計、貸方合計、残高が表示される
- [x] 期間を指定して絞り込みできる
- [x] 残高の推移をグラフで表示できる

| タスクID | タスク名 | 理想時間 | 担当 | 状態 |
|----------|---------|---------|------|------|
| LDG-003-01 | 日次残高照会ユースケース設計 | 2h | AI | 完了 |
| LDG-003-02 | GetDailyBalanceQuery 実装 | 2h | AI | 完了 |
| LDG-003-03 | GetDailyBalanceService 実装 | 4h | AI | 完了 |
| LDG-003-04 | 日次集計ロジック実装 | 4h | AI | 完了 |
| LDG-003-05 | 日次残高照会 API 実装（GET /api/daily-balance） | 3h | AI | 完了 |
| LDG-003-06 | 単体テスト作成 | 3h | AI | 完了 |
| LDG-003-07 | 日次残高照会画面 UI 実装 | 4h | AI | 完了 |
| LDG-003-08 | 残高推移グラフ実装（Recharts） | 4h | AI | 完了 |
| LDG-003-09 | フロントエンド API 連携 | 2h | AI | 完了 |
| LDG-003-10 | 統合テスト（E2E テスト） | 2h | AI | 完了 |
| | **小計** | **30h** | | |
...
- [x] コードレビューが完了している（PR #42 作成済み）
- [x] CI/CD パイプラインでビルドが成功している
- [x] ドキュメントが更新されている
- [x] テストカバレッジ 80% 以上を維持している
- [ ] リリースノートが作成されている（RELEASE-03 引き継ぎ）
...
|-------------|-------------|-----|------|--------|
| US-LDG-001 | 総勘定元帳照会 | 5 | ✅ 完了 | 2026-02-05 |
| US-LDG-003 | 日次残高照会 | 5 | ✅ 完了 | 2026-02-06 |
...
**Day 2 実績**: US-LDG-003（日次残高照会）を完了。バックエンド API、フロントエンド UI（Recharts グラフ）、E2E テスト（14件全パス）を実装。全ストーリー完了（10SP 達成）。
...
| 2026-02-06 | US-LDG-003（日次残高照会）完了、全ストーリー完了（10SP 達成）| Claude Opus 4.5 |
| 2026-02-06 | イテレーション完了、PR #42 作成、DoD チェックリスト更新 | Claude Opus 4.5 |
