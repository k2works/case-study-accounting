...
|----------|---------|---------|------|------|
| SYS-001-01 | 監査ログドメインモデル設計（AuditLog, AuditAction enum） | 2h | Claude | [ ] |
| SYS-001-02 | DB マイグレーション V15__create_audit_logs.sql | 2h | Codex | [ ] |
| SYS-001-03 | AuditLog ドメインモデル実装 | 2h | Codex | [ ] |
| SYS-001-04 | AuditLogRepository（Output Port）+ MyBatis Mapper 実装 | 4h | Codex | [ ] |
| SYS-001-05 | GetAuditLogsUseCase / Service 実装（検索・ページング） | 3h | Codex | [ ] |
| SYS-001-06 | 監査ログ記録サービス実装（AuditEventPublisher） | 4h | Codex | [ ] |
| SYS-001-07 | 既存サービスへの監査ログ記録組み込み（仕訳CRUD、認証） | 4h | Codex | [ ] |
| SYS-001-08 | 監査ログ照会 API 実装（GET /api/audit-logs） | 2h | Codex | [ ] |
| SYS-001-09 | バックエンド単体テスト | 4h | Codex | [ ] |
| SYS-001-10 | フロントエンド API クライアント実装 | 2h | Codex | [ ] |
| SYS-001-11 | 監査ログ照会 UI 実装（フィルター + テーブル + ページング） | 4h | Codex | [ ] |
| SYS-001-12 | 統合テスト（E2E テスト） | 2h | Claude | [ ] |
| | **小計** | **35h** | | |
